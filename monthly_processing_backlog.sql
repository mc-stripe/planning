...
datediff('day', pv.sales_activation_date, '2017-03-20') as days_since_activation,      -- UPDATE THIS TO THE LAST DAY OF PROCESSING
orig_activation,
first_year_npv,
...
inner join backlog_curve bc on bc.days_since_activation = datediff('day', pv.sales_activation_date, '2017-03-20')  -- HAVE TO SPECIFY THE DATE
...
select 
  ...
  to_char(date_trunc('month', capture_date)::date,'YYYY-MM-DD') as month,
...
case
 ...
 AS vertical,  
  ...
  to_char(date_trunc('month', orig_activation), 'YYYY-MM') as cohort,
  sum(total_npv) as total_npv_fixed_fx
FROM processing_volume pv
JOIN dim.merchants AS m ON pv.sales_merchant_id = m._id
LEFT JOIN country_code as cc ON m.sales__merchant_country = cc.country_code
LEFT JOIN team_role as usr ON usr.sales_owner = m.sales__owner
