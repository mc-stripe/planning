...
non_upsell_processing as (
   select
      ...
      nvl(adjusted_activation_date, sales__expected_go_live_date) as sales_activation_date,
      sales__expected_go_live_date as orig_activation, 
      sum(case 
         when datediff('d', sales__expected_go_live_date, capture_date) >= 0 and datediff('d', sales__expected_go_live_date, capture_date) < 366 and upsells.effective_activation_date::date is null then 1 * ap.npv_usd_fixed / 100 -- no upsell 99.9%
         when datediff('d', sales__expected_go_live_date, capture_date) >= 0 and datediff('d', sales__expected_go_live_date, capture_date) < 366 and datediff('d', upsells.effective_activation_date::date, capture_date) < 0 then 1 * ap.npv_usd_fixed / 100 -- upsell not started
         when datediff('d', sales__expected_go_live_date, capture_date) >= 0 and datediff('d', sales__expected_go_live_date, capture_date) < 366 and datediff('d', upsells.effective_activation_date::date, capture_date) >= 0 and datediff('d', upsells.effective_activation_date::date, capture_date) < 366 then (1 - pct_share_of_npv) * ap.npv_usd_fixed / 100 -- upsell and original sale overlap
         else 0 
